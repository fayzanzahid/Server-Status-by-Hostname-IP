...
<tr>
<th>Server Name</th><th>Server IP</th><th>Port</th><th>Status</th><th>ID</th><th>Action</th>
</tr>
...
if(empty($servers[0]) )
{
	echo '<h3 align="center">No Servers found.</h3>';
...
foreach ( $servers as $server ) 
{
	...
	$action = '<a href="?page=all-servers&id='.$id.'&action=edit" >Edit</a> |  <a href="?page=all-servers&id='.$id.'&delete=1" onclick=" return delete_confirm();">Delete</a>';
...
?>
<tr>
<td><?php echo $name; ?></td><td><?php echo $iphost; ?></td><td><?php echo $port; ?></td><td style="color:<?php echo $col; ?>;"><?php echo $status; ?></td><td><?php echo $id; ?></td><td><?php echo $action; ?></td>
</tr>
<?php
}
?>
...
$fp = @fsockopen($site, $port, $errno, $errstr, $timeout = 30);
if (!$fp) {
    return $status[0];
} else 
  { return $status[1];}
...
$iphost = sanitize_text_field($_POST['iphost']);
...
<tr>
<td class="text-left">Server Name:</td> <td class="text-left">
<li>
<input type="text" name="name"  />
    <span>Enter the Display Name for Server</span>
    </li>
...
function sship_edit_server()
{
global $wpdb;
		$id_update = sanitize_text_field($_GET['id']);

	if($_GET['faction'] == '_save'   )
    {
...
window.location = '<?php echo $_SERVER['PHP_SELF']; ?>?page=all-servers';
...
exit;
    ...
    $get_server_details = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix . "server_status where id = ".$id_update);
    
    foreach ($get_server_details as $get_server_detail)
    {
        $name = $get_server_detail->name;
        $ip = $get_server_detail->server;
        $port = $get_server_detail->port;
...
<form action="?page=all-servers&action=edit&faction=_save&id=<?php echo $id_update; ?>" method="post" class="form-style-7">
<h2><?php echo $msg; ?></h2>
<table class="table-fill">
...
<tr>
<td class="text-left">Server IP/Hostname:</td> <td class="text-left">
<li><input type="text" name="iphost" value="<?php echo $ip; ?>"  required="required"/>
    <span>Enter your Server's Hostname or IP</span>
    </li>
</td>
</tr>
...
<td class="text-left"></td> <td class="text-left">
<li>
<input type="submit" value="Update Server" /> <input type="reset" value="Reset"  /></td>
</li>
</tr>
...
foreach($servers as $server)
{
    ...
    <h3 style="color:<?php echo $col; ?> !important;"><?php echo $img; ?> &nbsp;<?php echo $name; ?> - (<?php echo $status; ?>)</h3>
...
$servers = $wpdb->get_results( 
	"
	SELECT * 
	FROM ".$wpdb->prefix . "server_status where id=".$atts
);
...
foreach($servers as $server)
{
	...
	{
		$col = '#3C0';
		$img = '<img style="float:left; width:15px;" src="'. plugin_dir_url( __FILE__ ) .'images/online.png"   />';
	}
	else
	{
		$col = '#F00';
		$img = '<img style="float:left; width:15px;" src="'. plugin_dir_url( __FILE__ ) .'images/offline.png"   />';
	}
...
?>
    ...
    <h3 style="color:<?php echo $col; ?> !important; font-size: 14px;"> <?php echo $img; ?>&nbsp;<?php echo $name; ?> - (<?php echo $status; ?>)</h3>
